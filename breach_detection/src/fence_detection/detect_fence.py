...
#img = cv2.imread('input/Eskild_fig_3_16.jpg', 0)
#img = cv2.imread('input/Eskild_fig_3_17.jpg', 0)
img = cv2.imread('input/Eskild_fig_3_18.jpg', 0)
...
peak_threshold = 800000 # Suitable for E_3_18
...
cv2.imwrite("output/05_shifted_fft_image.png", 
        shifted_fft * 255 / maxval)

# Save the fft peak mask.
cv2.imwrite("output/07_shifted_fft_peak_mask.png", 
        mask[:, :, 0] * 1.)

# Save the filtered image image.
minval, maxval, a, b = cv2.minMaxLoc(img_back[:, :, 0])
cv2.imwrite("output/10_low_pass_filtered_image.png", 
...
cv2.imwrite("output/20_low_pass_filtered_image.png", 
